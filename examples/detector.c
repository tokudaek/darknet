...
void test_detector_list(char *datacfg, char *cfgfile, char *weightfile,
		char *listpath, float thresh, float hier_thresh, char *outputdir,
		int fullscreen)
{
	FILE *stream;
	char *line = NULL;
	...
	while ((nread = getline(&line, &len, stream)) != -1) {
		...
		char *aux = basename(buff);
		char bname[128];
		strncpy(bname, aux, 128);
		int m = strlen(bname);
		bname[m-4] = '\0';	// strong assumption: extensions have three letters
		snprintf(outcsv, 256, "%s/%s.csv", outputdir, bname);

		if(access(outcsv, F_OK ) != -1) continue;
		...
		get_detections_csv(im, dets, nboxes, thresh, names, alphabet, l.classes,
				outdetections);
